<html>
    <head>
        ...
        <title>Customize Design Order Form</title>
        ...
        <form action = "#" method="post" name="myform" enctype="multipart/form-data">
            <div class="main-user-info">
                ...
                <div class="user-input-box">
                    ...
                    <textarea name = "comment" cols="33" rows="2" required class="nee">  </textarea>
                </div>
            ...
            <a href="OrderForm.php" class="gender-category" >
                    <p>Go Back</p>
                </a>
        </div>
        </form>
...
<?php
 ...
 if (isset($_POST['submit'])) {
     ...
     $filesize = $_FILES["did"]["size"];
     $tempname = $_FILES["did"]["tmp_name"];
     $folder = "customizeDesignphoto&matter/".$filename;

     move_uploaded_file( $tempname,  $folder);

     $filenamea =  $_FILES["m"]["name"];
     $filesizea = $_FILES["m"]["size"];
     $tempnamea = $_FILES["m"]["tmp_name"];
     $foldera = "customizeDesignphoto&matter/".$filenamea;

     move_uploaded_file( $tempnamea,  $foldera);
 
     $add = mysqli_query($conn, "insert into customizedesign (Name, Email, Address, ItemName, Quantity, ItemSize, ItemType, Limination, PrintingSide, PaperGSM, Design, Matter, Comment, Cost) 
     values ('$name', '$email', '$address','$iname','$quantity','$optionlist', '$type', '$limi', '$fix', '$gsm', '$folder', '$foldera', '$comment', '$wpn')") or die(mysqli_error($conn));
